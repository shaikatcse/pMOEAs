...
.core.Variable;
import ares.fbk.eu.pMOEA.jmetal
.encodings.solutionType.ArrayRealSolutionType;
import ares.fbk.eu.pMOEA.jmetal
.encodings.solutionType.BinaryRealSolutionType;
import ares.fbk.eu.pMOEA.jmetal
.encodings.solutionType.RealSolutionType;
import ares.fbk.eu.pMOEA.jmetal
.util.JMException;
 ...
 */
  ...
  int CIRCLE = 0;         // section type, 1 variable (di????metro)
  int HOLE_CIRCLE = 1;    // section type, 2 variable (di????metro externo y espesor)
  ...
  int Iw_=10; // warp modulus (m????dlo de alabeo)
  ...
  int Fyz_=16; //Fyz
  int Li_=17; // longitudinal de la barra r????gida en nodo i
  int Lj_=18; // longitud de la barra r????gida en nodo j
  ...
  int Ei_=5; // rigidez el????stica en nudo i
  int Ej_=6; // rigidez el????stica en nudo j

  // beams load index 
  int QH_=0; // hip????tesis de cargas
  ...
  int Qa_=6;   //distancia de aplicaci????n de la carga respecto al nudo i
  ...
  public void EBEsInitialize()  {
    ...
    // CON ESTO EVITO RECALCULARLOS CADA VEZ QUE SE BUSCA UNA SOLUCI????N
    // CONTAR EN PENALIZACI????N DE LA MATRIZ CA Y NO CN, CON ESTO
    ...
    // la forma de la secci????n determina las cantidades de variables
  ...
  public void evaluate(Solution solution) throws JMException {
    ...
    EBEsElementsTopology(solution); // transforma geometria a caracter????sticas mec????nicas
...
//  END OBJETIVES FUNCTION
  ...
  public void evaluateConstraints(Solution solution) throws JMException {
    ...
    // restricciones de relaci????n de forma en las paredes b/t
    for (int gr=0; gr<numberOfGroupElements_;gr++){
      ...
      if (Groups_[gr][SHAPE]==CIRCLE && Groups_[gr][CONSTRAINT]==0){
        ...
        constraint[con-1]=-ratio+Groups_[gr][RATIO_YZ]; // relaci????n entre altura y base
      }
        ...
        double ratio = x1/x2;
        constraint[con-2]=+ratio-Groups_[gr][RATIO_YZ]*0.75; // relaci????n entre altura y base maxima
        constraint[con-1]=-ratio+Groups_[gr][RATIO_YZ]*1.5; // relaci????n entre altura y base minima
      }
...
*/
        ...
        double ratio=x1/x2;
        constraint[con-4]=+ratio-Groups_[gr][RATIO_YZ]*0.75; // relaci????n entre altura y base maxima
        constraint[con-3]=-ratio+Groups_[gr][RATIO_YZ]*1.25; // relaci????n entre altura y base minima

        double tb1=-x3*15+x1;
        double tb2=+x3*30-x1;
        double ta1=-x4*10+x2;    
        double ta2=+x4*20-x2;    
        //double ta2=-x2/x4+27;//0.3*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS])); // relaci????n entre espesor de la placa y altura de lados
        constraint[con-2]=Math.min(tb1, tb2);//-x1/x3+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2)); // relaci????n entre espesor de la placa y altura de lados
        constraint[con-1]=Math.min(ta1, ta2);//-x2/x4+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2.0)); // relaci????n entre espesor de la placa y altura de lados
...
*/
        ...
        //double tb1=-x3*20+x1;
        //double tb2=-x1/x3+35;//0.6*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS])); // relaci????n entre espesor de la placa y altura de lados
        //double ta1=-x4*15+x2;    
        //double ta2=-x2/x4+27;//0.3*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS])); // relaci????n entre espesor de la placa y altura de lados
        constraint[con-2]=Math.min(tb1, tb2);//-x1/x3+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2)); // relaci????n entre espesor de la placa y altura de lados
        constraint[con-1]=Math.min(ta1, ta2);//-x2/x4+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2.0)); // relaci????n entre espesor de la placa y altura de lados
        ...
        x4=x[var-1].getValue(); // tickness along Z axis => tickness of heigth plate
        double ratio=x1/x2;
        constraint[con-4]=-ratio+Groups_[gr][RATIO_YZ]; // relaci????n entre altura y base maxima
        constraint[con-3]=+ratio-Groups_[gr][RATIO_YZ]*0.85; // /2.0 relaci????n entre altura y base minima
        constraint[con-2]=-x1/x3+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS])); // relaci????n entre espesor de la placa y altura de lados
        constraint[con-1]=-x2/x4+0.35*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS_CUT]*2.0)); // relaci????n entre espesor de la placa y altura de lados
      }
        ...
        x4=x[var-1].getValue(); // tickness along Z axis => tickness of heigth plate
        double ratio=x1/x2;
        constraint[con-4]=-ratio+Groups_[gr][RATIO_YZ]*0.85; // /2.0 relaci????n entre altura y base maxima
        constraint[con-3]=+ratio-Groups_[gr][RATIO_YZ]; // relaci????n entre altura y base minima
        constraint[con-2]=-x1/x3+0.35*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS_CUT]*2.0)); // relaci????n entre espesor de la placa y altura de lados
        constraint[con-1]=-x2/x4+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2.0)); // relaci????n entre espesor de la placa y altura de lados
      }
        ...
        x4=x[var-1].getValue(); // tickness along Z axis => tickness of heigth plate
        double ratio=x1/x2;
        constraint[con-4]=-ratio+Groups_[gr][RATIO_YZ]*0.1; // 2.0 relaci????n entre altura y base maxima
        constraint[con-3]=+ratio-Groups_[gr][RATIO_YZ]; // relaci????n entre altura y base minima
        constraint[con-2]=-x1/x3+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS_CUT]*2.0)); // relaci????n entre espesor de la placa y altura de lados
        constraint[con-1]=-x2/x4+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2.0)); // relaci????n entre espesor de la placa y altura de lados
      }
        ...
        constraint[con-4]=-ratio+Groups_[gr][RATIO_YZ]; // relaci????n entre altura y base maxima
        constraint[con-3]=+ratio-Groups_[gr][RATIO_YZ]; // relaci????n entre altura y base minima
        constraint[con-2]=-x1/x3+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2.0)); // relaci????n entre espesor de la placa y altura de lados
        constraint[con-1]=-x2/x4+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2.0)); // relaci????n entre espesor de la placa y altura de lados
      }
        ...
        double ratio=x1/x2;
        constraint[con-4]=-ratio+Groups_[gr][RATIO_YZ]; // relaci????n entre altura y base maxima
        constraint[con-3]=+ratio-Groups_[gr][RATIO_YZ]*0.9; // 2.0 relaci????n entre altura y base minima
        constraint[con-2]=-x1/x3+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2.0)); // relaci????n entre espesor de la placa y altura de lados
        constraint[con-1]=-x2/x4+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2.0)); // relaci????n entre espesor de la placa y altura de lados
      }
...
*/
    ...
    // RESTRICCIONES POR TENSI??N
    for(int hi=0; hi<numberOfWeigthHypothesis_; hi++){
      for(int gr=0; gr<numberOfGroupElements_; gr++){
          
    	  // RESTRICCIONES DEBIDO A LA TENSI????N DE TRACCI????N
    	  ...
    	  // RESTRICCIONES DEBIDO A LA TENSI????N DE COMPRESI????N
          ...
          // RESTRICCIONES DEBIDO A LA TENSI?????N DE CORTE
  ...
  public void EBEsElementsTopology(Solution solution) throws JMException{
    // asignaci????n de las variables para cada grupo
    // y determinaci????n de las caracter????sticas mec????nicas
  ...
  public void EBEsWeigthElement() throws JMException{
    ...
    for (int el=0; el<numberOfElements_;el++){
      ...
      //acumula cargas equivalentes en nudos en coordenadas LOCALES para la MISMA BARRA E HIP????TESIS
  ...
  public void EBEsCalculus() throws JMException{
    ...
    // Strain maximus  por hip????tesis de cargas     
    ...
    // Negative Strain residual  por hip????tesis de cargas
    StrainResidualMin_ = new double[numberOfWeigthHypothesis_];
    // Positive Strain residual  por hip????tesis de cargas
    StrainResidualMax_ = new double[numberOfWeigthHypothesis_];
    // Cut Strain residual  por hip????tesis de cargas
    ...
    for(int hi=0; hi<numberOfWeigthHypothesis_;hi++){
      ...
      for(int countIter=0;countIter<=NumIter;countIter++){
        ...
        //ARCHIVADO DE LA SOLUCI?????N DE ESTABILIDAD Y MEC?????NICA
      ...
      // para el proceso de optimizaci????n podemos prescindir
    ...
    // c????lculo de las tensiones, por barra calculo en los extremos
    // faltar????a calcular en los tramos y quedarme con el m????ximo
    // tensiones en i
    ...
    // tensiones m????ximas en cada barra
    // EBEsStrainMaxWhitElement();
    EBEsStrainMaxWhitGroup();

    // tensiones m????nimaa en cada barra
    // EBEsStrainMinWhitElement();
    EBEsStrainMinWhitGroup();

    // verificaci????n de las tensiones m????ximas respecto a las
  ...
  public void EBEsSteelingResults(int hi){
	  ...
	  for(int el = 0; el<Element_.length; el++){
	    	//esfuerzos en extremo i de la barra para el sistema principal de la secci????n
	      ...
	      //esfuerzos en extremo j de la elrra para el sistema principal de la secci????n
  ...
  public void EBEsMatrixWeight(int hi){
    ...
    // formaci????n del vector de fuerzas
  ...
  public void EBEsMatrixGlobalFactory(int countIter) throws JMException{
    ...
    for(int el=0; el<numberOfElements_; el++){
      ...
      // matriz de rotaci????n de ejes principales de secci????n a ejes locales (xp,yp)
      EBEsMatRot3DLpSaL(el);
      // matriz de rotaci????n de ejes locales a globales (x,y)
      EBEsMatRot3DLaG(el);
      //formaci????n y c????lculo de la matriz de rigidez de cada barra 3D en coordenadas globales
      EBEsMat3DGij();
  ...
  public void EBEsMatrixGlobalPenalization(){
    ...
    // penalizaci????n de la matriz asignando coacciones de nudos (apoyos)
    for(int i = 0; i<numberOfNodesRestricts_; i++){
      int no=(int)NodeRestrict_[i][0];
      //trasforma el n????????mero en c????digo texto caracterizando las coacciones;
      String strCxyz=String.valueOf((int)NodeRestrict_[i][1]);
      String str="";
      for(int j=numberOfLibertyDegree_;j>strCxyz.length();j--){str+="0";}
      strCxyz=str+strCxyz;
      // penalizaci????n de la matriz de rigidez

      char w0 = strCxyz.charAt(aX_); //sentido en X
      if(w0 == '1'){
        MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + aX_)] = 1.0E+35; // coacci????n r????gida en X
      } //coacci????n r????gida en X

      w0 = strCxyz.charAt(aY_); //sentido en Y
      if(w0 == '1'){
        MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + aY_)] = 1.0E+35; // coacci????n r????gida en Y
      } //coacci????n r????gida en Y

      w0 = strCxyz.charAt(aZ_); //sentido en Z
      if(w0 == '1'){
        MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + aZ_)] = 1.0E+35; // coacci????n r????gida en Z
      } //coacci????n r????gida en Z

      w0 = strCxyz.charAt(gX_); //rotaci????n alrededor del eje X
      if(w0 == '1'){
        MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + gX_)] = 1.0E+35; // coacci????n r????gida alrededor de X
      } //coacci????n r????gida de rotaci????n en X

      w0 = strCxyz.charAt(gY_); //rotaci????n alrededor del eje Y
      if(w0 == '1'){
        MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + gY_)] = 1.0E+35; // coacci????n r????gida alrededor de Y
      } //coacci????n r????gida en Y

      w0 = strCxyz.charAt(gZ_); //rotaci????n alrededor del eje Z
      if(w0 == '1'){
        MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + gZ_)] = 1.0E+35; // coacci????n r????gida alrededor de Z
      } //coacci????n r????gida de rotaci????n en Z
  ...
  public void EBEsOverloadWeightElement() throws JMException{
    ...
    for(int i = 0; i< numberOfWeigthsElements_;i++){
      ...
      //acumula cargas equivalentes en nudos en coordenadas GLOBALES la MISMA BARRA E HIP????TESIS
      //hipotesis asignada
      int hi = (int)OverloadInElement_[i][QH_];
      //identificaci????n de las barras cargadas
      int ni = (int)Element_[el][i_];
      int nj = (int)Element_[el][j_];
  ...
  public void EBEsWeightDistributedUniformly(int el, double[] LoadInElement_) throws JMException{
    ...
    //Qi() cargas equivalentes aplicadas en el extremo i de la barra con sentido y direcci????n del sistema GLOBAL
    //Qj() cargas equivalentes aplicadas en el extremo j de la barra con sentido y direcci????n del sistema GLOBAL
    //pi() cargas equivalentes aplicadas en el extremo i con sentido y direcci????n del sistema LOCAL de cada barra
    //pj() cargas equivalentes aplicadas en el extremo j con sentido y direcci????n del sistema LOCAL de cada barra
    ...
    if (vi == 0 && vj == 0) {
      //EMP-EMP, debe multiplicarse por la matriz de rotaci????n para
      ...
      //momento rotaci????n en x local
      Qi[gX_] = 0.0;
      Qj[gX_] = 0.0;
      //momento rotaci????n en y local
      Qi[gY_] = 0.0;
      Qj[gY_] = 0.0;
      //momento rotaci????n en z local
      Qi[gZ_] = 0.0;
      Qj[gZ_] = 0.0;
    }
    ...
    //matriz de rotaci????n de la barra del sistema principal al local
    EBEsMatRot3DLpSaL(el);

    //matriz de rotaci????n de la barra del local al global
    EBEsMatRot3DLaG(el);
  ...
  public void EBEsMatRot3DLpSaL(int e){
    //matriz de rotaci????n 3D en ejes principales "yp,zp" de la secci????n "S" a ejes LOCALES "y,z"
    //cuando los ejes principales de la secci????n est????n rotados un ????ngulo Beta respecto al sistema global
    ...
    //matriz de rotaci????n de desplazamientos locales a ejes globales XYZ si los ejes principales de la 
    //secci????n "yp,zp" coinciden con los ejes locales "y,z" de la barra
    ...
    //trasponer la matriz de rotaci????n
  ...
  public double [][]EBEsMatrizTraspuesta(double m[][]){
    ...
    for(int i = 0; i< row; i++){
      // cantidad de elementos de la 1ra dimensi????n
      for(int j = 0; j< col;j++){
        // cantidad de elementos de la 2ra dimensi????n
        mt[j][i] = m[i][j];
      }
    }
  ...
  public void EBEsEcuationSolution(int hi) throws JMException{
    ...
    // PROGRAMACI?????NN TOMO III
    // Ram????n Arguellez ?????lvarez
...
// Salto4:
  ...
  public void EBEsMat3DL_iRig_jRig(int e) throws JMException{
    ...
    // i: rigid
    // j: rigid
    // esfuerzos en nudo j por reacci????n de desplazamientos en i + esfuerzo en i
    ...
    // esfuerzos en nudo i por reacci????n de desplazamientos en j
    Kij[0][0] = E * S / Lij;
    Kij[0][1] = 0;
    Kij[0][2] = 0;
    Kij[0][3] = 0;
    ...
    // esfuerzos en nudo j por reacci????n de desplazamientos en i
    Kji[0][0] = E * S / Lij;
    Kji[0][1] = 0;
    Kji[0][2] = 0;
    Kji[0][3] = 0;
    ...
    // esfuerzos en nudo i por reacci????n de desplazamientos en j + esfuerzo en j
    Kjj[0][0] = E * S / Lij;
    Kjj[0][1] = 0;
    Kjj[0][2] = 0;
    Kjj[0][3] = 0;
  ...
  public void EBEsMatRot3DLaG(int e) throws JMException{
    // matriz de rotaci????n 3D de desplazamientos de ejes Locales a Generales
    int i, j;
    // cosenos directores de x local respecto al sistema global
    double lx; // cosenos directo respecto del eje local x (coincidente con el eje de la barra) y el eje X Global
    double mx; // cosenos directo respecto del eje local x (coincidente con el eje de la barra) y el eje Y Global
    ...
    if(lx == 0 && mx == 0){
      // indeterminaci????n por ser la barra con eje local x // al eje global z
      // cosenos directores de x local respecto al sistema global XYZ
  ...
  public double []EBEsMatrizVectorMultiplicar(double [][]s, double[]t) throws JMException{
    ...
    for(f =0; f<s.length; f++){ // cantidad de elementos de la 1ra dimensi????n
      r[f] = 0;
      for(c=0; c<t.length; c++){ // cantidad de elementos de la 2ra dimensi????n
        r[f] = r[f] + s[f][c] * t[c];
      } // next c
    }// next f
  ...
  public double [][]EBEsMatrizMultiplicar(double [][]s, double[][]t) throws JMException{
    ...
    for(f=0; f<s.length; f++){ // cantidad de elementos de la 1ra dimensi????n
      for(c=0; c<s[f].length; c++){ //cantidad de elementos de la 2ra dimensi????n
        r[f][c]=0;
        for(q=0; q<s[f].length; q++){ //cantidad de elementos de la 2ra dimensi????n
  ...
  public double [][]EBEsMatrixAdd(double [][]s, double[][]t) throws JMException{
	    ...
	    for(int f=0; f<s.length; f++){ // cantidad de elementos de la 1ra dimensi????n
	      for(int c=0; c<t.length; c++){ //cantidad de elementos de la 2ra dimensi????n
	        r[f][c] = s[f][c] + t[f][c];
	      } // Next c
	    } // Next f
  ...
  public void EBEsNodesEquilibrium3D(int hi) throws JMException{
    ...
    for(int ba=0; ba<Element_.length;ba++){
      ...
      //Proyecci????n de los esfuerzos de barras sobre los ejes generales
      //en el nudo i
...
// [0]: Tensi????n normal de compresi????n
// [1]: Tensi????n normal de tracci????n
// [2]: Tensi????n tangencial
...
// ver C????lculo de estructuras VIII.35
    ...
    for(int hi=0; hi<numberOfWeigthHypothesis_; hi++){
      for(int ba=0; ba<numberOfElements_; ba++){
        ...
        //Tensi????n normal en x debido a esf. axial x
        // Buckling coeficient omega
        // coeficiente de mayoraci??n de Pandeo
        double omega  = BucklingOmega(Nxx, Groups_[idx], Element_[ba]);
        Sxx=omega*Nxx/A;

        if(Element_[ba][Vij_] != 11)
        {
          //tensiones de flexi????n alrededor del eje z
          //fibra superior e inferior
          y1=Groups_[idx][uY_];
          Sxzu=Mxz*y1/Iz;
          Sxzd=-Sxzu;

          //tensiones normales de flexi????n alrededor del eje y
          ...
          //momento est????tico respecto a z
          //espesores de las paredes
          //se desprecia el cord????n superior, revisar
          //momento est????tico respecto a y
          //espesores de las paredes
          //se desprecia el cord????n lateral

          //TENSIONES TANGENCIALES
          //tensi????n de corte en y
          TQxy=Qxy*Az/(z*Iz);
          // CORTE TRANSVERSAL eje Z
          // Tensi????n de corte en z
          TQxz=Qxz*Ay/(y*Iy);
 ...
 public double BucklingOmega(double Nxx, double[] G, double[] B) throws JMException{
	 ...
	 double ?? = 1.0; // coeficiente de Pandeo
  ...
  if(Nxx < 0.0 && G[AREA] > 0.0 && lBuckling)
  {
      ...
      double ??o  = B[L_] * G[BLijY_] / iy;
      // esbeltez maxima entre para los ejes y, z
      ??o = Math.min(??o, B[L_] * G[BLijZ_] / iz);
	      ...
	      {
	          // M??todo Domke - Tabla Lamda0  - Lamda
	    	  // double ?? = -2.3443 + 1.07817 * ??o - 0.0036181 * Math.pow(??o, 2) + 0.000008209862 * Math.pow(??o, 3) - 0.00000001058458 * Math.pow(??o, 4) + 0.000000000007147864 * Math.pow(??o, 5) - 0.00000000000000196410755 * Math.pow(??o, 6);
	          // coeficiente omega de mayoraci??n por efecto de Pandeo
	          // double ratio = ??o / ??;
	          // ?? = Math.pow(ratio, 2.0);
              ...
              if ( ?? <= 150)
              {
            	  ?? = 1.113 + 0.0070516 * ?? - 0.000132108 * Math.pow(??, 2.0) + 0.000002106132 * Math.pow(??, 3.0) - 0.00000000397368332151 * Math.pow(??, 4.0);
              }
              else ?? = 25.0;
	      }
          ...
          else if(G[TypeMaterial_] == 10){  // HL-7 Hormig??n   7 MN/m2
              System.out.println("Error in " + G[INDEX_] + " group, the material number " + G[TypeMaterial_] + " is not implemented");
          }

          else if(G[TypeMaterial_] == 12){  // HL-17 Hormig??n 17 MN/m2
              System.out.println("Error in " + G[INDEX_] + " group, the material number " + G[TypeMaterial_] + " is not implemented");
          }

          else if(G[TypeMaterial_] == 14){  // HL-21 Hormig??n 21 MN/m2
              System.out.println("Error in " + G[INDEX_] + " group, the material number " + G[TypeMaterial_] + " is not implemented");
          }
          ...
          else if(G[TypeMaterial_] == 20)  // Wood hard an halt-hard
	      {
             ...
             if (??<=150)
             {
                 ?? = 1.048 + 0.005524 * ?? - 0.000101666 * Math.pow(??, 2.0) + 0.00000301687 * Math.pow(??, 3.0) - 0.000000004366246 * Math.pow(??, 4.0);
             }
             else ?? = 25.0;
	      }
      }
  ...
  public void EBEsTransversalSectionCircular(int gr, double d) throws JMException{
    ...
    // momento est????tico respecto de Z ->  Az
    Groups_[gr][Az_] = Am * Math.pow(y, 2.0);
    // momento est????tico respecto de Y ->  Ay
  ...
  public void EBEsTransversalSectionRectangle(int gr, double y, double z) throws JMException{
    ...
    //z:  lado de la base del rect????ngulo
    //y:  altura del rect????ngulo
    // section
    Groups_[gr][AREA]=z*y;
    // mementum inertia Iz
  ...
  public void EBEsTransversalSectionHoleRectangle(int gr, double y, double z, double ey, double ez) throws JMException{
    ...
    //  y: lado de la secci????n rectangular
    //  z: lado de la secci????n rectangular en sentido z
    // ey: espesor de cada pared se la secci????n hueca en y
    // ez: espesor de cada pared se la secci????n hueca en z
    ...
    zl=zi/2.0+ez/2.0;
    // momento est????tico del cord????n lateral
    el=al*Math.pow(zl, 2.0);
    // area del cordon extremo sup e inferior
    ae=ey*zi/2.0;
    // distancia del baricentro al centro 
    ze=zi/4.0;
    //momento est????tico de 2 cordones extremos
    ee=2.0*ae*Math.pow(ze, 2.0);

    // momento est????tico respecto de Y ->  Ay
    Groups_[gr][Ay_]=el+ee;

    // area cord????n superior
    as=z*ey;
    //distancia del baricentro al centro 
    ys=yi/2.0+ey/2.0;
    //momento est????tico al cord????n superior
    es=as*Math.pow(ys, 2.0);
    //area cord????n lateral
    al=ez*(yi/2.0);
    //distancia del baricentro al centro 
    yl=yi/4.0;
    //momento est????tico de 2 cordoneslaterales
    el=2.0*al*Math.pow(yl, 2.0);
    // momento est????tico respecto de Z ->  Az
    Groups_[gr][Az_]=es+el;

    // area de la secci????n hueca
    double Ai=zi*yi;
    // area total
    double At=z*y;
    // thw solid area A
  ...
  public void EBEsTransversalSection_I_Single(int gr, double y, double z, double ey, double ez) throws JMException{
    ...
    // la orientaci????n del perfil es con el alma coincidente con el eje Y
    ...
    //lados de la secci????n hueca virtual
    yi=y-2*ey;
    zi=z-ez;
    ...
    // area del alma
    al=yi*ez;
    //distancia del baricentro del alma al baricentro de la secci????n completa 
    zl=ez/2;
    // momento est????tico del alma
    el=al*Math.pow(zl, 2.0);
    ...
    // distancia del baricentro al centro 
    ze=z/4.0;
    //momento est????tico de los 2 cordones extremos
    ee=ae*Math.pow(ze, 2.0);

    // momento est????tico total respecto de Y ->  Ay
    Groups_[gr][Ay_]=el+ee;
  ...
  public void EBEsTransversalSection_L_Double(int gr, double y, double z, double ey, double ez) throws JMException{
    ...
    // la orientaci????n del perfil es con las almas coincidente con
  ...
  public void EBEsStrainMaxWhitElement() throws JMException{
    // determinaci????n de las tensiones m????ximas entre los extremos
    for(int hi=0; hi<numberOfWeigthHypothesis_; hi++){
  ...
  public void EBEsStrainMinWhitElement() throws JMException{
    // determinaci????n de las tensiones minimas entre los extremos
    // de las barras para cada agrupaci????n
    for(int hi=0; hi<numberOfWeigthHypothesis_; hi++){

      // tensiones normales m????nimas
      // en nudo menor numeraci????n i
  ...
  public void EBEsPrintArchTxtElements() throws JMException{
    try {
      ...
      // impresion de la las caracter????sticas de las barras
  ...
  public String EBEsReadProblems() {
      ...
      // iteraciones = total evaluaciones / tama??o poblaci??n
