...
package ares.fbk.eu.pMOEA.jmetal.core;
...
public class SolutionSet implements Serializable {
  ...
  public boolean add(Solution solution) {
    if (solutionsList_.size() == capacity_) {
     ...
     try {
		throw new JMException("Capacity is full");
	} catch (JMException e) {
  ...
  */
  ...
  public SolutionSet(SolutionSet solutionSet){
	  solutionsList_ = new ArrayList(solutionSet.size());
	  for(int i =0 ;i<solutionSet.size();i++){
		  solutionsList_.add(solutionSet.get(i));
	  }
	  this.capacity_= solutionSet.size();
  }
  
  public void removeSolutionSetSolutions(SolutionSet solutionSet){
	  List<Solution> SolutionsToRemove = new ArrayList();
	  
	  for(int i=0;i<solutionSet.size();i++){
		  SolutionsToRemove.add(solutionSet.get(i));
	  }
	  solutionsList_.removeAll(SolutionsToRemove);
	  this.capacity_ = this.capacity_ - solutionSet.size();
	  
  }
  
  public void addAll(SolutionSet solutionSet){
	  List<Solution> SolutionsToAdd = new ArrayList();
	  
	  for(int i=0;i<solutionSet.size();i++){
		  SolutionsToAdd.add(solutionSet.get(i));
	  }
	  solutionsList_.addAll(SolutionsToAdd);
	  this.capacity_= this.capacity_+solutionSet.size();
  }
} // SolutionSet
