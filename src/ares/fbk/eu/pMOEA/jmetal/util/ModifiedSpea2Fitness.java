...
.core.SolutionSet;
import ares.fbk.eu.pMOEA.jmetal
.util.comparators.DominanceComparator;
import ares.fbk.eu.pMOEA.jmetal
.util.comparators.FitnessComparator;
 ...
 */
public class ModifiedSpea2Fitness {
  ...
  /**
   * stores a <code>Comparator</code> for preferred objective Space checking
   */
  private static final Comparator ObjectiveSpaceComparator_ = new ObjectiveSpaceAndDominanceComparator();
  
  /**
   * stores a <code>Comparator</code> for dominance checking
   */
  private static final Comparator dominanceComparator_ = new ObjectiveSpaceAndDominanceComparator();
   ...
   */
  public ModifiedSpea2Fitness(SolutionSet solutionSet) {
  ...
  //shahriar
  public void fitnessAssign(SolutionSet solutionSet, SolutionSet totalPopulaion, int evaluations, int maxEvaluations) {
	  double [] strength    = new double[solutionSet_.size()];
	    double [] rawFitness  = new double[solutionSet_.size()];
	    double kDistance                                          ;
	     ...
	     * stores a <code>Comparator</code> for preferred objective Space checking
	     */
	    Comparator ObjectiveSpaceComparator_ = new ObjectiveSpaceAndDominanceComparator();
	    
	    /**
	     * stores a <code>Comparator</code> for dominance checking
	     */
	    Comparator dominanceComparator_ = new ObjectiveSpaceAndDominanceComparator();
	  ...
	  //adapted from pAGE strategy
	  	...
	  	if( rm.nextDouble() < Math.pow( (evaluations/(double)maxEvaluations) , 10)){
	  			comparator_ = ObjectiveSpaceComparator_;
	  	}else{
	  		comparator_ = dominanceComparator_;
	  	}
	    ...
	    for (int i = 0; i < solutionSet_.size(); i++) {
	      for (int j = 0; j < solutionSet_.size();j++) {
	        if (comparator_.compare(solutionSet_.get(i),solutionSet_.get(j))==-1) {
	          strength[i] += 1.0;
	        } // if        
	      } // for
	    } // for
	    ...
	    //shahriar
	    double []   dominateMeByGP    = new double[solutionSet.size()];
	    for (int i = 0; i < totalPopulaion.size(); i++) {
	        for (int j = 0; j < solutionSet.size();j++) {
	          if (comparator_.compare(totalPopulaion.get(i),solutionSet.get(j))==-1) {
	        	  dominateMeByGP[j] ++;
	          }
	        }
	    }
